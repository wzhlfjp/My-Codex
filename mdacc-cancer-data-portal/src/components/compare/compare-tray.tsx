...
export function CompareTray() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-3 z-40 px-3 sm:bottom-4">
      <div className="pointer-events-auto mx-auto max-w-4xl rounded-xl border border-slate-300 bg-white p-3 shadow-lg">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-700">Compare Tray</p>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {selection.items.map((item) => (
                <span
                  key={item.id}
                  className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-slate-50 px-2 py-0.5 text-xs text-slate-700"
                >
          ...
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            ...
            {canCompare && compareUrl ? (
              <Link
                href={compareUrl}
                className="rounded-md bg-slate-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
              >
                Compare
              </Link>
            ) : (
              <span className="rounded-md border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs text-slate-500">
                Compare
              </span>
            )}
          </div>
