...
const dataPath = path.join(
  projectRoot,
  "src/data/generated/businessSchoolRankings.generated.json"
);
...
const documentedNoRankingYears = new Set(
  (raw.noRankingYears ?? []).map((yearStatus) => yearStatus.year)
);
const errors = [];
const warnings = [];
...
for (const yearData of data.years) {
  ...
  if (documentedNoRankingYears.has(yearData.year)) {
    assert(
      ...
      `${yearData.year}: documented no-ranking year should not contain fabricated ranking rows.`
    );
    assert(
      typeof yearData.statusNote === "string" && yearData.statusNote.length > 0,
      `${yearData.year}: documented no-ranking year must include a status note.`
    );
  }
  ...
  if (yearData.entries.length === 0 && documentedNoRankingYears.has(yearData.year)) {
    warnings.push(`${yearData.year}: no annual U.S. News MBA ranking publication verified; documented explicitly.`);
  } else if (yearData.entries.length === 0) {
    errors.push(`${yearData.year}: no verified ranking rows and no no-ranking-year documentation.`);
  } else if (yearData.entries.length < 20) {
    warnings.push(
      `${yearData.year}: available source data include ${yearData.entries.length} displayable top-20 rows; documented in coverage summary.`
    );
  }
...
const sourceFilesToScan = [
  path.join(projectRoot, "src/hooks/useInterpolatedBusinessRankings.ts"),
