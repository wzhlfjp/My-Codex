...
const dataPath = path.join(
  projectRoot,
  "src/data/generated/lawSchoolRankings.generated.json"
);
...
for (const yearData of data.years) {
  ...
  if (yearData.entries.length === 0 && documentedNoRankingYears.has(yearData.year)) {
    warnings.push(`${yearData.year}: no annual U.S. News law ranking publication verified; documented explicitly.`);
...
const sourceFilesToScan = [
  path.join(projectRoot, "src/hooks/useInterpolatedLawRankings.ts"),
