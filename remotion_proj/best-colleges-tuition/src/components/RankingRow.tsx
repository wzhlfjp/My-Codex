...
const valueToWidth = (value: number) =>
  Math.max(
    ...
    Math.min(
      ...
      (value / chartLayout.maxAxisValue) * chartLayout.barWidth
    )
  );
...
export const RankingRow = ({
  ...
  const width = entry.hasTuition
    ? valueToWidth(entry.animatedVisualValue)
    : valueToWidth(chartLayout.missingValue);
  const valueLabel = entry.tuitionFees === null ? "N/A" : formatCurrency(entry.tuitionFees);
  const stats = [
    ...
    >
      ...
      >
        <div
          style={{
            ...
            borderRadius: 999,
            background: entry.hasTuition ? entry.color : "rgba(255, 253, 246, 0.16)",
            boxShadow: entry.hasTuition
      ...
      <div
        style={{
          ...
          top: 5,
          color: entry.hasTuition ? "#fffdf6" : "rgba(255, 255, 255, 0.48)",
          fontSize: 20,
          fontWeight: 920,
